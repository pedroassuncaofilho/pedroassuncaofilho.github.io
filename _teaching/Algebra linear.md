---
title: "Álgebra Linear"
collection: teaching
type: "Curso para Engenharia Civil 2025/2"
permalink: /teaching/2025-spring-teaching-4
venue: "Instituto Federal de Goiás, DAA"
date: 2025-01-01
location: "Formosa, Goiás"
---
[Link moodle](https://moodle.ifg.edu.br/course/view.php?id=44026)
[Link Suap](https://suap.ifg.edu.br/)

# Álgebra Linear

##  Horários das Aulas 

- **Sextas-feiras:** 19:00 às 22:00 (Engenharia Civil 2ª período)


##  Ementa

Matrizes; Sistemas de equações lineares; Determinante e matriz inversa; Espaço vetorial;
Transformações lineares; Autovalores e Autovetores; Diagonalização de operadores; Produto interno.

##  Objetivos

Capacitar o aluno a utilizar ferramentas de Álgebra Linear para a resolução de problemas relacionados
a Construção Civil.

##  Bibliografia

### Textos de Referência
- **BOLDRINI, J. L.; COSTA, S. I. R.; FIGUEIREDO, V. L.; WETZLER, H.G.** *Álgebra Linear.* São Paulo:
Harbra, 1980.
- **KOLMAN, B. HILL. D. R.** * Introdução à álgebra linear com aplicações.*  Rio de Janeiro: LTC, 2006.
- **STEINBRUCH, Alfredo; WINTERLE, Paulo.** *Álgebra Linear.* 2. ed. São Paulo: Pearson Education do Brasil, 2014.

### Textos Complementares
- **BOLDRINI, J.L; COSTA, R.I.S; FIGUEIREDO, L.V; WETZLER, G.H.** *Álgebra Linear.* 3. ed. São Paulo: Harbra, 1986.
- **CALLIOLI, C.A; COSTA, R.F.; DOMINGUES, H.** *Álgebra linear e aplicações.* São Paulo: Saraiva, 2005.
- **LIMA, Elon Lages.** *Álgebra Linear.* Rio de Janeiro: IMPA. Coleção Matemática Universitária, 9. ed. 2016.
- **HOFFMAN, Kennet; KUNZE, Ray.** *Álgebra Linear.* São Paulo: Polígono, 1971.
- **POOLE, David.** *Álgebra Linear.* São Paulo: Pioneira Thomson Learning, 2004.


## Canais de Acesso

- **E-mail do professor:** pedro.filho@ifg.edu.br
- **Sala do professor:** Prédio do Departamento DAA.(Atendimento ao discente previamente marcado)
- **Monitoria:** 2025/2

##  Avaliações

### 1ª Prova
- **Conteúdo:** Sistemas lineares, operações elementares, matrizes, cálculo de inversa, determinantes
- **Data:** 03/10/25 (sexta-feira)

### 2ª Prova
- **Conteúdo:** Espaços vetoriais
- **Data:** 28/11/25 (sexta-feira)

##  Critérios para Aprovação

- **Faltas acima de 25% da carga horária** → Reprovado(a) por falta
- **Média parcial ≥ 6,0** → Aprovado(a) (desde que não reprovado por falta)
- **Média parcial < 6,0** → Avaliação final (desde que não reprovado por falta)
  - Neste caso: **Média final ≥ 5,0** → Aprovado(a)

## Notas de Aula e Listas de Exercícios

> **ATENÇÃO:** As notas de aula apresentam os principais resultados discutidos em aula, bem como demonstrações e exemplos. Apesar de servirem de apoio, **elas não substituem os livros-texto!**

### 1. Sistemas Lineares
- **Lista de exercícios 1 - Operações elementares e sistemas lineares**
- Exercícios Anton, seção 1.2
- Exercícios Anton, seção 1.3
- Exercícios Anton, seção 1.5

### 2. Matrizes
- **Lista de exercícios 2 - Matrizes**

### 3. Determinantes
- **Lista de exercícios 3 - Determinantes**

### 4. Espaços e Subespaços Vetoriais
- Combinação linear, independência linear, base
- Capítulos 4 e 5 do livro do Anton

##  Conteúdo de Terceiros

### Vídeo-aulas UNIVESP
- Aula sobre matrizes: [parte 1], [parte 2]

### Vídeo-aulas (Prof. Possani, USP)
- [Vetores no R³ (revisão de geometria analítica)]
- [Independência / Dependência linear]
- [Base, dimensão e revisão de sistemas lineares]
- [Mudança de base] - **SOMENTE ATÉ 34:50**

### Conteúdo IMPA/Obmep (nível ensino médio)
- [Matrizes e sistemas lineares]
- [A geometria das transformações lineares]
- [Operando com transformações lineares]

##  Aplicações da Álgebra Linear

A Álgebra Linear aplica-se a uma infinidade de situações! Veja abaixo referências com algumas delas:

### Referências no Anton e Rorres
- Seções 1.8, 1.9, 4.12, 5.4, 7.4, 9.6 e todo o capítulo 10 do livro "Álgebra Linear com aplicações" (10ª edição)

### Referências no David Lay
- Seções 1.9, 2.7, 2.8, 4.8, 4.9, 5.6, 5.7, 6.5, 6.6, 6.8, 7.5 do livro "Álgebra Linear e suas aplicações"

### Outras Fontes
- No Google é fácil encontrar inúmeras aplicações!


======
