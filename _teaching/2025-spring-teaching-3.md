---
title: "Cálculo Numérico"
collection: teaching
type: "Curso para Engenharia Civil 2026/1"
permalink: /teaching/2026-spring-teaching-1
venue: "Instituto Federal de Goiás, DAA"
date: 2026-01-01
location: "Formosa, Goiás"
---
[Link moodle](https://moodle.ifg.edu.br/course/view.php?id=44026)
[Link Suap](https://suap.ifg.edu.br/)



# Ementa de Disciplina

## Informações Gerais

| Campo | Descrição |
|-------|-----------|
| **Disciplina** | Cálculo Numérico |
| **Formação** | Básica |
| **Série/Período** | 5° |
| **Carga Horária** | 54h |
| **Pré-requisito** | - |
| **Regime** | Obrigatória |

## Ementa

Zeros de funções. Zeros de Polinômios. Sistemas de Equações Lineares. Inversão de Matrizes. Ajuste
de Curvas. Interpolação. Integração Numérica. Resolução Numérica de Equações diferenciais
ordinárias.
## Objetivo

Capacitar o aluno a utilizar ferramentas de Álgebra Linear para a resolução de problemas relacionados a Construção Civil.

## Critérios de Avaliação

### Instrumentos Avaliativos

| Instrumento | Quantidade | Pontuação | Peso |
|-------------|------------|-----------|------|
| **Avaliações** | 2 | 10,0 pontos cada | 100% |
| **Listas de Exercícios** | Contínuas | Complementar | - |

### Cálculo da Média Final

**Média Final = (Avaliação 1 + Avaliação 2) ÷ 2**

- **Aprovação**: Média Final ≥ 6,0
- **Recuperação**: 4,0 ≤ Média Final < 6,0
- **Reprovação**: Média Final < 4,0

## Recursos Complementares

### Videoaulas Recomendadas

<!-- Adicione aqui as URLs das videoaulas do YouTube -->
**Slides:**
- [Slide 1 ](https://www.dropbox.com/scl/fi/f1gpy8n4ugkpype66cj5b/Aula9-1-1.pdf?rlkey=18358s7iozy7vdq0zjq14vt1r&st=139zjxh7&dl=0)
- [Título da Aula - Canal/Professor](URL_DO_VIDEO)



**Videos:**
- [Sitema de Numeração - UFPR/Prof.João godoi](https://www.youtube.com/watch?v=3jTueAA6Fhw&list=PLsfUsqON-YybXUTyvm_xy8J8VpHRvpG_z)
- [Título da Aula - Canal/Professor](URL_DO_VIDEO)

<!-- 
Para adicionar novos vídeos, use o formato:
- [Nome descritivo da aula - Nome do canal](URL_completa_do_YouTube)
-->
### Exercícios

- ✏️ [Livro - Colaborativo](https://www.dropbox.com/scl/fi/b9fj2ka6x3t9ln2rmik6x/main.pdf?rlkey=tmfbhppyo55d7llpobh2hm40v&st=dmm3gr30&dl=0)  
  -  [Lista 1  ](https://www.dropbox.com/scl/fi/enppf4g55emnlvnjl6zud/Modelo_lista_de_exercic_os-1.pdf?rlkey=eg4q06cicagt5k4s0iqydcwld&st=jux4mlw3&dl=0)  
## Bibliografia
 
### Bibliografia Básica
 
1. **BARROSO, L. C. et al.** *Cálculo numérico*. São Paulo: Harbra, 1987.
2. **FRANCO, N. B.** *Cálculo numérico*. São Paulo: Pearson Prentice Hall, 2006.
3. **LOPES, V. L. R.; RUGGIERO, M. A. G.** *Cálculo numérico: aspectos teóricos e computacionais*. 2. ed. São Paulo: Makron Books, 1996.
 
### Bibliografia Complementar
 
1. **ARENALES, S.; DAREZZO, A.** *Cálculo numérico: aprendizagem com apoio de software*. São Paulo: Thomson Learning, 2008.
2. **BURDEN, R. L.; FREITAS, J. D.** *Análise Numérica*. 8. ed. São Paulo: Cengage Learning, 2008.
3. **CLÁUDIO, D. M.; MARINS, J. M.** *Cálculo numérico computacional: teoria e prática*. 3. ed. São Paulo: Atlas, 2000.
4. **DORN, W. S.; McCRACKEN, D. D.** *Cálculo Numérico com Estudos de Casos em Fortran IV*. Rio de Janeiro: Ramon Raz Livros, 1978.
5. **PENNINGTON, R. H.** *Introductory computer methods and numerical analysis*. New York: The Macmillan Company, 1965.
 
---
*Documento gerado em: 14/04/2026*
